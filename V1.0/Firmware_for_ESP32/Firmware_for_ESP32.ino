...
byte get_input_from_PS2_keyboard() {
...
byte get_PS2_input_from_the_ps2_keyboard_mvn_bcg() {
  byte data_from_cntrl_and_keyb;
  while (rec_d == false) {
    mvn_bcg();
...
void get_to_ps2_input_from_the_ps2_keyboard_tab() {
  finish_input = false;
  rec_d = false;
  byte inp_frm_cntr_ps2_kbrd = 0;
  while (finish_input == false) {
    inp_frm_cntr_ps2_kbrd = get_input_from_PS2_keyboard();
...
void get_to_starred_ps2_input_from_the_ps2_keyboard_tab() {
  finish_input = false;
  rec_d = false;
  byte inp_frm_cntr_ps2_kbrd = 0;
  while (finish_input == false) {
...
void typing_inscription(){
  display_background_while_handling_record();
  tft.setTextSize(1);
  tft.setTextColor(0xffff);
...
void display_decrypting_record_inscription(){
  display_background_while_handling_record();
  tft.setTextSize(1);
  tft.setTextColor(0xffff);
  disp_text_b_w("Decrypting the record...", 5);
  disp_text_b_w("Please wait for a while.", 17);
}
...
void select_login(byte what_to_do_with_it) {
  ...
  while (continue_to_next == false) {
    byte input_data = get_input_from_PS2_keyboard();
    if (input_data > 0) {
      ...
      if (input_data == 13 || input_data == 133) { // Enter
        ...
        if (what_to_do_with_it == 1 && continue_to_next == false) {
          continue_to_next = true;
          display_decrypting_record_inscription();
...
void enter_title_for_login(int chsn_slot) {
  ...
  get_to_ps2_input_from_the_ps2_keyboard_tab();
  if (act == true) {
...
void update_login_and_tag(int chsn_slot, String new_password) {
  ...
  tft.setTextSize(1);
  tft.setTextColor(0xffff);
  disp_text_b_w("Editing Login In The Slot N" + String(chsn_slot) + "...", 5);
  disp_text_b_w("Please wait for a while.", 17);
...
void delete_login(int chsn_slot) {
  ...
  tft.setTextSize(1);
  tft.setTextColor(0xffff);
  disp_text_b_w("Deleting Login From The Slot N" + String(chsn_slot) + "...", 5);
  disp_text_b_w("Please wait for a while.", 17);
...
void update_credit_card_and_tag(int chsn_slot, String new_pin) {
  ...
  tft.setTextSize(1);
  tft.setTextColor(0xffff);
  disp_text_b_w("Editing Credit Card In The Slot N" + String(chsn_slot) + "...", 5);
  disp_text_b_w("Please wait for a while.", 17);
  clear_variables();
...
void delete_credit_card(int chsn_slot) {
  ...
  tft.setTextSize(1);
  tft.setTextColor(0xffff);
  disp_text_b_w("Deleting Credit Card From The Slot N" + String(chsn_slot) + "...", 5);
  disp_text_b_w("Please wait for a while.", 17);
...
void update_note_and_tag(int chsn_slot, String new_content) {
  ...
  tft.setTextSize(1);
  tft.setTextColor(0xffff);
  disp_text_b_w("Editing Note In The Slot N" + String(chsn_slot) + "...", 5);
  disp_text_b_w("Please wait for a while.", 17);
...
void delete_note(int chsn_slot) {
  ...
  tft.setTextSize(1);
  tft.setTextColor(0xffff);
  disp_text_b_w("Deleting Note From The Slot N" + String(chsn_slot) + "...", 5);
  disp_text_b_w("Please wait for a while.", 17);
...
void update_phone_number_and_tag(int chsn_slot, String new_phone_number) {
  ...
  tft.setTextSize(1);
  tft.setTextColor(0xffff);
  disp_text_b_w("Editing Phone Number In The Slot N" + String(chsn_slot) + "...", 5);
  disp_text_b_w("Please wait for a while.", 17);
  clear_variables();
...
void delete_phone_number(int chsn_slot) {
  ...
  tft.setTextSize(1);
  tft.setTextColor(0xffff);
  disp_text_b_w("Deleting Phone Number From The Slot N" + String(chsn_slot) + "...", 5);
  disp_text_b_w("Please wait for a while.", 17);
...
void show_moving_background() {
  ...
  bool break_the_loop = false;
  while (break_the_loop == false) {
    byte input_data = get_PS2_input_from_the_ps2_keyboard_mvn_bcg();
    if (input_data > 0) {
      break_the_loop = true;
    }
  }
...
void display_background_while_handling_record(){
  if (display_pattern_while_adding_and_printing_record == true){
    byte chosen_pattern = generate_random_number() % 5;
    if (chosen_pattern == 0){
...
void unlock_midbar() {
  ...
  get_to_starred_ps2_input_from_the_ps2_keyboard_tab();
  tft.fillScreen(0x0000);
  draw_47px_midbar_inscription();
  tft.setTextSize(2);
  tft.setTextColor(0xffff);
...
void get_plaintext_for_custom_key_encr_serp_from_keyb(byte enckey[]) {
  ...
  get_to_ps2_input_from_the_ps2_keyboard_tab();
  if (act == true) {
    encrypt_serp_with_custom_key(input_from_the_ps2_keyboard, enckey);
  }
  return;
}
...
void get_plaintext_for_custom_key_encr_aes_from_keyb(byte enckey[]) {
  ...
  get_to_ps2_input_from_the_ps2_keyboard_tab();
  if (act == true) {
    encrypt_aes_with_custom_key(input_from_the_ps2_keyboard, enckey);
  }
  return;
}
...
void split_for_aes_in_cbc_cust_key_decryption(char ct[], int ct_len, int p) {
  ...
  if (br == false) {
    ...
    int i = 0;
    aes_context ctx;
    set_aes_key(&ctx, custom_aes_and_serp_in_cbc_key, aes_mode[m]);
    aes_decrypt_block(&ctx, ret_text, cipher_text);
    incr_custom_aes_and_serp_in_cbc_key();
    if (decract > 2) {
      for (int i = 0; i < 16; i++){
...
const char EEPROM_HEADER[] = "Midbar";

struct DeviceConfig {
  char wifi_ssid[100];
  char wifi_password[100];
  char api_key[100];
  char database_url[100];
  unsigned int iterations;
};
...
bool isConfigStored() {
  ...
  header[6] = '\0';
  EEPROM.end();
  return String(header) == EEPROM_HEADER;
}

void clearDispForConfig() {
  tft.fillScreen(TFT_BLACK);
  tft.setTextColor(TFT_WHITE);
  tft.setTextSize(2);
  tft.setCursor(0, 10);
  disp_centered_text("Midbar Configuration", 10);
  for (int i = 0; i < 192; i++) {
    for (int j = 0; j < 47; j++) {
      if (midbar_inscr_47px_high[i][j] == 1)
        tft.drawPixel(i + 63, j + 60, 0xffff);
    }
  }
  for (int i = 0; i < 192; i++) {
    for (int j = 0; j < 47; j++) {
      if (midbar_inscr_47px_high[i][j] == 1)
        tft.drawPixel(i + 64, j + 61, 0xffff);
    }
  }
  for (int i = 0; i < 192; i++) {
    for (int j = 0; j < 47; j++) {
      if (midbar_inscr_47px_high[i][j] == 1)
        tft.drawPixel(i + 65, j + 62, 0xffff);
    }
  }
  for (int i = 0; i < 192; i++) {
    for (int j = 0; j < 47; j++) {
      if (midbar_inscr_47px_high[i][j] == 1)
        tft.drawPixel(i + 66, j + 63, current_inact_clr);
    }
  }
}

void requestAndSaveConfig() {
  
  bool confirmed = false;
  
  while (!confirmed) {
    clearDispForConfig();
    disp_centered_text("Paste Access Point Name", 140);
    disp_centered_text("to the Serial Terminal", 160);
    
    Serial.println("Enter SSID (Access Point Name):");
    while (!Serial.available()) {}
    String ssid = Serial.readString();
    ssid.trim();
    
    clearDispForConfig();
    disp_centered_text("Paste", 140);
    disp_centered_text("Access Point Password", 160);
    disp_centered_text("to the Serial Terminal", 180);
    
    Serial.println("Enter Access Point Password:");
    while (!Serial.available()) {}
    String password = Serial.readString();
    password.trim();
    
    clearDispForConfig();
    disp_centered_text("Paste Web API Key", 140);
    disp_centered_text("to the Serial Terminal", 160);
    
    Serial.println("Enter Web API KEY:");
    while (!Serial.available()) {}
    String apiKey = Serial.readString();
    apiKey.trim();
    
    clearDispForConfig();
    disp_centered_text("Paste Database URL", 140);
    disp_centered_text("to the Serial Terminal", 160);
    
    Serial.println("Enter Database URL:");
    while (!Serial.available()) {}
    String databaseUrl = Serial.readString();
    databaseUrl.trim();
    
    clearDispForConfig();
    disp_centered_text("Enter iterations", 140);
    disp_centered_text("to the Serial Terminal", 160);
    
    Serial.println("Enter iterations (default: 20451):");
    while (!Serial.available()) {}
    String iterationsInput = Serial.readString();
    iterationsInput.trim();
    unsigned int iterations;
    if (iterationsInput.toInt() > 0) {
      iterations = iterationsInput.toInt();
    } else {
      iterations = 20451;
      Serial.println("Invalid input. Using default value: 20451");
    }
    ...
    Serial.println("Access Point Password: " + password);
    Serial.println("Web API KEY: " + apiKey);
    Serial.println("Database URL: " + databaseUrl);
    ...
    if (confirmation.equalsIgnoreCase("Y")) {
      ...
      strncpy(deviceConfig.api_key, apiKey.c_str(), sizeof(deviceConfig.api_key) - 1);
      strncpy(deviceConfig.database_url, databaseUrl.c_str(), sizeof(deviceConfig.database_url) - 1);
      deviceConfig.iterations = iterations;
      ...
      deviceConfig.api_key[sizeof(deviceConfig.api_key) - 1] = '\0';
      deviceConfig.database_url[sizeof(deviceConfig.database_url) - 1] = '\0';
      ...
      EEPROM.put(6, deviceConfig);
      EEPROM.commit();
      EEPROM.end();
      
      clearDispForConfig();
      disp_centered_text("Configuration Saved", 140);
      disp_centered_text("to EEPROM", 160);
      Serial.println("Configuration saved to EEPROM");
      delay(2000); // Display the message for 2 seconds
    } else {
      clearDispForConfig();
      disp_centered_text("Configuration", 140);
      disp_centered_text("not confirmed", 160);
      disp_centered_text("Please try again", 180);
      Serial.println("Configuration not confirmed. Please enter values again.");
      delay(2000);
    }
  }
}
...
void loadConfig() {
  EEPROM.begin(EEPROM_SIZE);
  EEPROM.get(6, deviceConfig);
  EEPROM.end();
  
  // Copy the loaded values to the global variables and filter the strings
  WIFI_SSID = filterString(String(deviceConfig.wifi_ssid));
  WIFI_PASSWORD = filterString(String(deviceConfig.wifi_password));
  API_KEY = filterString(String(deviceConfig.api_key));
  DATABASE_URL = filterString(String(deviceConfig.database_url));
  iterations = (unsigned int) deviceConfig.iterations;
  
  // Print the loaded configuration values to Serial
  Serial.println("Loaded configuration:");
  Serial.println("WIFI_SSID (Access Point Name): " + WIFI_SSID);
  Serial.println("WIFI_PASSWORD (Access Point Password): " + WIFI_PASSWORD);
